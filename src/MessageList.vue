<template>
  <div class="sc-message-list" v-on:scroll="scrollFunction" ref="scrollList" :style="{backgroundColor: colors.messageList.bg}">
  ...
  methods: {
    ...
    scrollFunction(){
      if(this.$refs.scrollList.scrollTop == 0){
        let old = this.$refs.scrollList.scrollHeight

        Event.$emit("messagePaging")
          setTimeout(() => {
              this.$refs.scrollList.scrollTop = this.$refs.scrollList.scrollHeight-old
          }, 200)
